...
func (b *Bucket) OpenUploadStreamWithID(fileID interface{}, filename string, opts ...*options.UploadOptions) (*UploadStream, error) {
	...
	file, err := b.parseUploadOptions(fileID, filename, opts...)
	if err != nil {
		return nil, err
	}

	return newUploadStream(file, b.chunksColl, b.filesColl), nil
}
...
func (b *Bucket) OpenDownloadStream(fileID interface{}) (*DownloadStream, error) {
	...
	return b.openDownloadStream(bsonx.Doc{bsonx.Elem{Key: "_id", Value: id}})
}
...
func (b *Bucket) OpenDownloadStreamByName(filename string, opts ...*options.NameOptions) (*DownloadStream, error) {
	...
	return b.openDownloadStream(bsonx.Doc{bsonx.Elem{Key: "filename", Value: bsonx.String(filename)}}, findOpts)
}
...
func (b *Bucket) Rename(fileID interface{}, newFilename string) error {
	...
	res, err := b.filesColl.UpdateOne(ctx,
		...
		bsonx.Doc{bsonx.Elem{Key: "$set", Value: bsonx.Document(bsonx.Doc{
			bsonx.Elem{Key: "filename", Value: bsonx.String(newFilename)}})}},
	)
	if err != nil {
		return err
	}
...
func (b *Bucket) parseUploadOptions(fileID interface{}, filename string, opts ...*options.UploadOptions) (*File, error) {
	file := &File{
		ID:        fileID,
		Filename:  filename,
	...
	if uo.ChunkSizeBytes != nil {
		file.ChunkSize = *uo.ChunkSizeBytes
	}
	if uo.Registry == nil {
...
func (b *Bucket) openDownloadStream(filter interface{}, opts ...*options.FindOptions) (*DownloadStream, error) {
	...
	var file File
	if err := bson.Unmarshal(cursor.Current, &file); err != nil {
		return nil, err
	}

	return newDownloadStream(&file, b.chunksColl, b.filesColl), nil
}

func (b *Bucket) uploadFromStream(us *UploadStream, source io.Reader) error {
	if err := us.SetWriteDeadline(b.writeDeadline); err != nil {
		_ = us.Close()
		return err
	}
...
func (b *Bucket) downloadToStream(ds *DownloadStream, stream io.Writer) (int64, error) {
	if err := ds.SetReadDeadline(b.readDeadline); err != nil {
		_ = ds.Close()
		return 0, err
	}
...
func (b *Bucket) findChunks(ctx context.Context, fileID interface{}) (*mongo.Cursor, error) {
	...
	chunksCursor, err := b.chunksColl.Find(ctx,
		bsonx.Doc{bsonx.Elem{Key: "files_id", Value: id}},
